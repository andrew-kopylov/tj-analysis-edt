...
Процедура ЗагузитьФайлыНаКлиент(Команда)
	...
	Если Не ПустаяСтрока(АдресХранилищаАрхива) Тогда
		...
		// Метод НачатьПолучениеФайлов устарел начиная с 8.3.15,
		// рекомендуется использовать метод НачатьПолучениеФайловССервера.
		//НачатьПолучениеФайлов(ОписаниеОповещения, ПолучаемыеФайлы,, Истина);
		
		ПараметрыДиалога = Новый ПараметрыДиалогаПолученияФайлов(, Ложь);
...
Процедура ЗагузитьФайлыНаКлиентНаСервере(Знач МассивИдентификаторов)
	...
	Для Каждого ИдентификаторСтроки Из МассивИдентификаторов Цикл
		ИмяФайла = Объект.ФайлыЖурналов.НайтиПоИдентификатору(ИдентификаторСтроки).Путь;
...
Процедура ЗагузитьФайлыНаКлиентЗавершение(Результат, ПараметрыОповещения) Экспорт
	Сообщить("Получен файл на клинте");		
КонецПроцедуры
...
&НаСервереБезКонтекста
Процедура ЗаписатьКонфигурациюТЖ(ПутьКФайлу, ТекстКонфТЖ, СоздатьКопию = Истина)
...
&НаКлиентеНаСервереБезКонтекста
Функция ПодробноеОписаниеОшибки(ИнформацияОбОшибке = Неопределено)
	
	Если ИнформацияОбОшибке = Неопределено Тогда
		ИнформацияОбОшибке = ИнформацияОбОшибке();
	КонецЕсли;

	// Устаревший вариант, начиная с 8.3.17:
	
	//@skip-warning
	ОписаниеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()); 
	
	// Используется, начиная с 8.3.17:
